...
export const book30Chapters: Chapter[] = [
  makeChapter({
    ...
    moves: [
      ...
      { title: "Note what the two conditions rule out", body: "The two conditions are what separate delusion from ordinary error, and both are needed. An ordinary mistake can be dropped as soon as it is corrected, because nothing in the person is holding it — delusion requires that the belief agree with desire, so there is something with an interest in keeping it. And it requires a semblance functioning as evidence, so the person can point at grounds. Take away either and you have a man who is merely wrong." },
      { title: "See why this makes it a book about arguments", body: "And once the definition includes a false ground, the treatment has to be argumentative — which is why the next section takes the syllogism apart rather than exhorting anybody. A delusion is a piece of reasoning the person has never written out. Nothing else in the Quarter of Perils is treated this way, because nothing else in it is built out of a bad inference the holder could recognise if he saw it laid out." },
    ],
    closer: [
  ...
  makeChapter({
    ...
    moves: [
      ...
      { title: "Take the first example apart carefully", body: "The correction to the first premise is exact and it is worth having. Cash is better than credit only when the two are equal in amount and in kind — and nobody actually believes it otherwise, which is why every merchant in the world gives one dirham now for ten later and considers himself shrewd for it. So the deluded man is holding a principle he already abandons whenever anything is at stake, and has only failed to notice where else it applies." },
      { title: "Follow the second example", body: "And the second is worse, because both premises fail rather than one. Worldly pleasure is not certain — the merchant's toil is certain and his profit is not, the student's effort is certain and his attainment is not, the hunter walks and may catch nothing. Which means the trade being condemned is exactly the trade everybody agrees is prudent: certain effort now against a probable return later. The deluded man is calling imprudent the one thing he calls prudent everywhere else." },
      { title: "Note the general mechanism", body: "And the mechanism named at the end covers both cases and a great many more. A general saying is taken up in its familiar form, while the qualification the speaker had in mind goes unsaid — cash is better than credit means better than a credit like it, and nobody says so because nobody needs to. The deluded man is not inventing anything. He is running a true saying past the boundary where it was true, and the boundary was never written down." },
    ],
    closer: [
  ...
  makeChapter({
    ...
    moves: [
      ...
      { title: "Follow the reasoning about past kindness", body: "The syllogism here is one almost everybody has run: God has been good to me so far; a benefactor is a well-wisher; a well-wisher goes on doing good. So the past is being used to underwrite the future, and the whole weight rests on reading present ease as evidence of favour. Which is exactly what desire wants read, and exactly what the analogy is designed to unsettle." },
      { title: "Take the two slaves at their own pace", body: "And the analogy is the sharpest thing in the book. A man has two young slaves, one he loves and one he does not. The one he loves is kept from play, held to his lessons, refused the delicacies that would harm him, and made to swallow bitter medicine. The one he does not care about is left alone to live as he likes. And the neglected boy concludes he is the favourite — because nothing is being withheld from him. Every fact is correct and the inference is exactly backwards." },
      { title: "See where the failure actually occurs", body: "And the failure is located precisely: the other reading is available to the mind. Nothing hides it and no argument rules it out. It simply does not agree with what the man wants, so it is not taken up — and Satan works through the desire, tilting the heart toward whichever of two live readings suits it. Ghazali says outright that this is the definition of delusion, which means the fault is not in the reasoning but in which conclusion was allowed to stand." },
    ],
    closer: [
  ...
  makeChapter({
    ...
    moves: [
      ...
      { title: "Note the inversion that gives it away", body: "The single strongest observation in the section is a comparison of two generations. The ancestors, with the utmost scrupulousness, were afraid. The descendants, with the utmost laxity, feel entirely safe. If inherited standing worked as claimed, the confidence should track the conduct — and it runs precisely opposite. Ghazali calls this the furthest reach of being deluded about God, and the evidence is nothing more than putting the two attitudes side by side." },
      { title: "Follow the cases and the general argument", body: "And the cases are chosen to close the position from the highest possible ground. Noah wished to take his own son aboard and was told the boy was not of his household. Abraham asked forgiveness for his father and it did not avail. And the Prophet was permitted to visit his mother's grave and not permitted to ask forgiveness for her, and sat and wept until the people around him wept. Then the general argument, which is short: if love passed down, so would hatred — and no bearer of burdens bears another's." },
    ],
    closer: [
  ...
  makeChapter({
    ...
    moves: [
      ...
      { title: "Take the hired man seriously", body: "The story is the cleanest test in the book. A man is hired to repair vessels for a generous employer who keeps his word and pays above the agreement. He arrives, smashes every vessel, then sits down to wait for his wages, on the grounds that the employer is generous. Every fact about the employer is true. Nobody would call the man's expectation hope, and the point is that nothing about the case changes when the employer is God." },
      { title: "Note the criterion, which is behavioural", body: "And the criterion offered is a test on effects rather than on feelings, which is what makes it usable. An expectation that pushes a person toward repentance or toward more diligence is hope. One that produces slackness and settles into idleness is delusion. Fear and hope are described as a driver and a leader — so anything that does not move a person is not one of them, whatever it feels like from inside." },
      { title: "See what the substitution accomplished", body: "And the framing is worth registering: the thing itself is wishing upon God, and what Satan changed was only its name. Which explains why the fault is so hard to challenge — argue with it and you appear to be arguing against hope, one of the two things every believer is told to have. The renaming did not create the error; it made the error unattackable." },
    ],
    closer: [
  ...
  makeChapter({
    ...
    moves: [
      ...
      { title: "List the mechanisms", body: "The mechanisms include seeing what is blameworthy as good, confusing what one does for oneself with what one does for God, choosing the less important over the more important, choosing voluntary acts over obligations, and leaving the substance for the shell." },
      ...
      { title: "Note why these four classes", body: "The four classes are scholars, worshippers, Sufis, and the wealthy — and the choice is deliberate. Each begins from something genuinely good: real learning, real worship, a real path, real resources. Delusion needs a semblance that can pass for a proof, and only somebody with something actual in his hands has the material to be deceived about. A man with nothing good in his life is not a candidate for this book." },
      { title: "Follow the five mechanisms", body: "And the five mechanisms are worth memorising, because they recur in every class. Seeing what is objectionable as good. Failing to tell what you do for yourself from what you do for God. Leaving the more important for the less. Leaving what is obligatory for what is optional. And leaving the kernel for the husk. Three of the five involve no wrongdoing at all — only a misallocation, which is why the deluded man can review his conduct and find nothing to answer for." },
    ],
    closer: [
  ...
  makeChapter({
    ...
    moves: [
      ...
      { title: "Take the prescription story at its own pace", body: "The parable is exact in every detail, which is what makes it land. A man has an illness that only a compound remedy will cure. He travels, finds a skilled physician, learns the ingredients and the quantities and where they come from and how they are prepared, writes it out in a fine hand, comes home, recites it and teaches it to the sick. A thousand copies and a thousand people cured — and he is exactly as ill as when he set out." },
      { title: "Note the argument from the wording", body: "And the criterion is drawn from what the verse does not say. He has prospered who purifies it. Not: who learned how to purify it, or wrote the knowledge down, or taught it to people. The whole weight of the section rests on a piece of close reading, which is appropriate in a chapter about a class of men who read for a living." },
      { title: "Follow the answer to Satan's objection", body: "And the objection is the one a learned reader will already be forming: knowledge is rewarded in itself, and here are the reports saying so. The answer is a point about consistency rather than about knowledge. Whoever transmitted the merits of learning also transmitted what is said about the corrupt scholar — and you cannot take one half of a source as authoritative and treat the other half as though it were not there." },
    ],
    closer: [
  ...
  makeChapter({
    ...
    moves: [
      ...
      { title: "Take the weeding image as the governing one", body: "The weeding image is the one that explains the whole class. A man cuts the tops and ends off the weeds instead of pulling them out, so the roots strengthen and the growth comes back — because the beds in which the sins are planted are the blameworthy traits in the heart. Which means the outward acts genuinely were cut. Nothing about his effort was fake, and the effort was applied at the wrong level." },
      { title: "Note what the four surface images add", body: "And the four images that follow all make one point from different angles: a cesspit plastered white, a grave adorned outside with carrion inside, a dark house with a lamp on the roof, and a man expecting the king who whitewashes his door and leaves the dungheaps in the middle of the house. The last is the sharpest, because the whitewashing was a real preparation for a real visit — and it was done at the one point the visitor will not be looking." },
      { title: "Follow the medical version", body: "And the medical form supplies what the images cannot: a mechanism. A man with scabies is given an ointment for the skin and a draught to cut off what is producing it. He takes the ointment, drops the draught, and goes on consuming whatever feeds it. So he keeps painting the surface while the eruption keeps arriving from inside — which is a description of a life of scrupulous outward correction, and of why it never finishes." },
    ],
    closer: [
  ...
  makeChapter({
    ...
    moves: [
      ...
      { title: "Note the self-sealing structure", body: "This class is the hardest of the seven because of its shape. They believed themselves too elevated to be tried by pride, envy and display — so they stopped looking. And the conceit that produced the exemption is precisely what conceals the exemption, since a man who has decided he is beyond a fault has no reason to inspect himself for it. The fault protects itself by removing the instrument that would find it." },
      { title: "Take the relabelling seriously", body: "And the relabelling is worth quoting because every clause of it is defensible. This is not pride but the honour of religion, and manifesting the nobility of knowledge, and defeating the innovators — and if I wore poor clothing my enemies would gloat, and my humiliation would be a humiliation of Islam. A man who says that is not lying about anything checkable. He has supplied a public reason for a private appetite, and the reason is good." },
      { title: "Note the test that settles it", body: "And the test offered is the best diagnostic in the book, because it removes the person from the picture. Would you be equally angry if a different scholar were attacked, or passed over for the position? If not — and if you might even be a little pleased — then the anger was never for religion. It is answerable privately, immediately, and it cannot be argued with, which is why Umar's reply is placed beside it: we are a people God honoured through Islam, and we seek honour in nothing else." },
    ],
    closer: [
  ...
  makeChapter({
    ...
    moves: [
      ...
      { title: "Note how the weeding image is refined", body: "The image returns and is sharpened, and the refinement is the point. This man is not neglecting the weeds — he searches out every one he can see and pulls it. What he does not do is look for what has not yet broken the surface, and fine shoots have already spread beneath the soil. So he is a careful worker who has finished, and the ruin is proceeding under a field he has correctly judged to be clean." },
      { title: "Take the test about authorship", body: "And the test is devastating precisely because it is small. If somebody claimed his book and scratched his name off it, it would gnaw at him — and he knows, as a matter of doctrine, that the reward for its usefulness goes to whoever actually wrote it, and that God is perfectly aware who that was. Neither of those beliefs stops the gnawing. Which is exactly what the test is for: the belief was never what was driving the work." },
      { title: "Follow the detail into the writing itself", body: "And the detail runs down into the craft. Praising himself in the book directly, or by attacking somebody else so the attack does it for him. Attributing the weak opinions to whoever said them and leaving the good ones unattributed, so they are taken for his. Then the image that closes it — a man who steals a shirt and cuts it into a coat so that nobody will recognise it. The alteration is the labour, and the labour is what proves the theft." },
    ],
    closer: [
  ...
  makeChapter({
    ...
    moves: [
      ...
      { title: "Take the analogy of the fatal illness", body: "The analogy is precise about what is being neglected and what is being pursued. The man has an illness that will kill him and needs to learn its remedy and take it — and instead spends day and night on the rulings for a condition he cannot possibly have, on the grounds that some woman may one day ask him about it. The study is legitimate, the question may genuinely arise, and none of that touches the fact that he is dying of something else." },
      { title: "Note the principle, which is legal", body: "And the principle stated is not an exhortation but a ruling: occupying yourself with a communal obligation before you have completed an individual one is disobedience — and it holds even where the intention behind the study is entirely sound. Which is the doctrine from the book on knowledge applied as a verdict rather than as advice, and it means good intentions do not settle the question of what a person should be doing this year." },
    ],
    closer: [
  ...
  makeChapter({
    ...
    moves: [
      ...
      { title: "Note the diagnosis of the straying party", body: "The fault of the party in error is located exactly where the first section said delusion always sits: they did not suspect their own view, and did not first settle what counts as a proof and how one is built. So one of them took a semblance for a proof and a proof for a semblance — which is not a failure of sincerity or of effort, but of never having established the standard before applying it." },
      { title: "Take the harder case seriously", body: "The party with the correct doctrine is deluded from a different direction, which is what makes this section important. They assumed that argument was the highest religious work and the greatest means of drawing near to God. From there, they concluded that religion is incomplete without technical investigation, and that a person who accepts God and His Messenger without constructing a proof is either not a believer or not fully one. Being right about doctrine did not protect them from being wrong about the purpose of a life." },
    ],
    closer: [
      { title: "The counsel that follows", body: "Ghazali gives different advice to someone who has not devoted his life to study: do not wade into the competing schools or attempt to judge all their arguments. Hold that God is one and nothing is like Him, and that His Messenger told the truth in what he reported. Follow the way of the early community and believe what came in the Book and prophetic practice without probing technical details. Concentrate on God-consciousness, avoiding wrongdoing, fulfilling obligations, and treating people with kindness." },
  ...
  makeChapter({
    ...
    moves: [
      ...
      { title: "Follow the inference at the top of the class", body: "The severest case is the one closest to this book's own subject, which is worth registering. A man who speaks about fear, hope, patience, gratitude, trust, sincerity — and supposes that speaking about them has made him so. The inference he runs is genuinely persuasive: I could not have gone this deep into the science of love unless I loved, could not have found the hidden faults of the soul unless I were free of them. Every step feels like evidence and none of it is." },
      { title: "Note who is called a devil among humankind", body: "And the harshest phrase in the book is reserved not for the ignorant but for the preachers who went after novelty — extravagant sayings, phrases outside the measure of Law and reason, rhyming and flourishes aimed at multiplying cries and raptures in the gathering. He calls them devils among humankind, who erred and led others astray. The severity is proportionate to the position: they had the one instrument that reaches people who will read nothing." },
      { title: "See what the collectors have in common", body: "And the last group all substitute possession for understanding — memorising the ascetics' sayings without grasping them, chasing rare elevated chains so as to be able to say whom they narrate from, spending a life on grammar and rare words and supposing themselves the pillars of the community. In each case something real has been accumulated. What has not happened is any of it reaching the person doing the accumulating." },
    ],
    closer: [
  ...
  makeChapter({
    ...
    moves: [
      ...
      { title: "Note the diagnosis of misallocated scruple", body: "The first observation is the one that exposes the whole class: the same man treats remote possibilities of impurity in water as near, and near possibilities of unlawfulness in food as remote. One scruple runs hot and the other cold, in the same person, on the same day. Which cannot be explained by carefulness — and the counter-example is Umar, who made ablution from a Christian woman's jar and gave up whole areas of the lawful for fear of the unlawful, with the two settings exactly reversed." },
      { title: "Follow it into the prayer", body: "And the same misallocation runs through the prayer itself. Anxiety about the intention until the congregation has gone and the time has run out. Anxiety about pronouncing the opening until the phrase itself is deformed. And then complete inattention for everything that follows — while the man takes the effort spent at the start as what distinguishes him from ordinary worshippers." },
      { title: "Take the letter as the summary image", body: "And the letter is the sharpest of the two images. A master writes his slave a letter of instructions. The slave does not try to understand it or act on it; he memorises it, goes on doing the opposite of what he was told, and recites it aloud in a fine voice a hundred times a day. Nothing about his devotion to the letter is feigned, which is why he cannot be reached by being told to take it more seriously." },
    ],
    closer: [
  ...
  makeChapter({
    ...
    moves: [
      { title: "Describe the imitation", body: "They copied truthful Sufis in dress, bearing, expressions, manners, observances, and terminology. They sat on the mat with the head bowed as if absorbed in thought, sighed deeply, and lowered the voice. Having adopted these outward signs, they assumed they had become Sufis. Yet they had not undertaken the struggle, discipline, and watchfulness of the heart that begin the path." },
      ...
      { title: "Reach the last group", body: "The last group abandoned the Law and erased the distinction between lawful and unlawful. They claimed that God had no need of their actions, that hearts could not be purified of appetite and so the obligation was impossible, or that bodily deeds carried no weight because their hearts were already in the divine presence while their bodies remained in the world." },
      { title: "Take the old woman at the review", body: "The review story is the most complete argument in the book. She learns the mail and the helmet, the warriors' verses and their tunes, the swagger and the hand movements — and presents herself to have her name entered in the register. Every observable marker is correct. Then she is ordered stripped of the armour and put to single combat, and what is found is a weak old woman who cannot carry the weight of what she was wearing. The test is not of the signs but of what the signs were signs of." },
      { title: "Read the robes back to their origin", body: "And the detail about the robes is the finest thing in the section. Having found plain dress too hard, they went to costly patched robes, fine cloths and dyed mats — wearing what is finer than silk while supposing the colour and the patching made them Sufis. And the origins are recovered: the colour was to save frequent washing, the patches were because the garments had torn. Both marks began as consequences of poverty and have been reproduced as its insignia." },
      { title: "Note the three arguments of the last group", body: "And the last group supplies three arguments for setting the Law aside, each of which is a doctrine rather than a lapse. God has no need of their actions. Hearts cannot be cleared of appetite, so the charge is impossible. Deeds of the limbs carry no weight, and their hearts are already in the presence while their bodies happen to be here. Every one is answerable, and none of them is the sort of thing a man drifts into — they were reasoned to." },
    ],
    closer: [
  ...
  makeChapter({
    ...
    moves: [
      ...
      { title: "Examine the object", body: "Others earned lawfully and spent on mosques, yet Ghazali identifies two possible delusions. Poor people in the same neighborhood may have a more urgent claim on the money, while giving to a visible building feels easier and more rewarding. The money may also be spent on gilding and ornament that distracts the hearts and eyes of those who come to pray." },
      ...
      { title: "Note the test of the inscription", body: "The test is the cleanest in the section and it costs nothing to apply. Require one of them to spend a single dinar without his name being cut into the stone, and it weighs on him — his soul will not consent. And God is equally aware of the gift whether the name is there or not. So the inscription is doing no work at all in the direction the man says his money is going, which settles what the money was actually for." },
      { title: "Follow the two faults in mosque-building", body: "And the second case is subtler because the money is lawful and the object is a mosque. Two things are wrong with it. There may be poor people in the same street for whom it matters more — and giving to them is harder precisely because a mosque is visible and they are not. And the money goes into gilding and ornament that then occupies the eyes and hearts of the people praying in it, so the gift damages the act it was built to house." },
      { title: "Take the conditions attached to charity", body: "And the last group gives genuinely, and attaches conditions that give the game away: they want a crowded assembly, and grateful recipients who will mention it; they dislike giving in secret; and they count it as an offence when a poor man keeps quiet about what he was given. Then the detail that closes it — repeating the pilgrimage while the neighbours go hungry, which is the fourth mechanism from section six in its plainest form." },
    ],
    closer: [
  ...
  makeChapter({
    ...
    moves: [
      ...
      { title: "State the required posture", body: "The sound posture is to see the whole state as God's grace. At the same time, the person fears that an unnoticed trait may have covered the heart or that the state may be taken away in an instant. Gratitude therefore does not become self-certification, and fear of the final outcome prevents carelessness." },
      { title: "Take Satan's last speech seriously", body: "The speech is constructed to be true. You have exhausted me and got away by your intelligence; I overcame many of the saints and could not overcome you; how great your rank must be, that God strengthened you to defeat me. Every clause flatters and none of it is obviously false — the man really did escape, and the escape really was unusual. Which is why it works, and why it is placed last." },
      { title: "Follow the trap through both exits", body: "And the trap has two exits and both are covered. If he believes the speech and marvels at himself, the self-admiration is the utmost delusion — and Satan's reply names it exactly: when you supposed that by your knowledge you had escaped me, by your ignorance you fell into my snares. And if he does not marvel, knowing the escape was God's doing and not his, something still threatens him: delusion by the grace itself, resting on the generosity and feeling safe from the devising." },
      { title: "Note the posture the book ends on", body: "And the posture required at the end is genuinely uncomfortable, which is the right way for this book to close. Witness all of it as grace — and then fear that some trait may have closed over the heart without his noticing, and fear that the state could be taken away in the blink of an eye. Neither secure nor heedless. After seventeen sections of people who arrived somewhere and settled, the only safe position turns out to be the one that never settles." },
    ],
    closer: [
...
export const book30Journeys: Journey[] = [
  ...
  },
  {
    ...
    nodes: [
      ...
      node("weed-the-root", "Weed at the root", "Cut tops grow back", "Faults that keep returning are planted by traits that were never treated.", "This does not make outward obedience unimportant.", 8, "cultivate"),
