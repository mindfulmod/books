...
export const book11Chapters: Chapter[] = [
  makeChapter({
    ...
    moves: [
      ...
      { title: "See why the four chapters are cumulative", body: "The layering is the book's whole design and it is worth naming what it implies. Nothing in the first chapter is suspended when somebody else sits down; company adds seven duties on top of it, hosting adds more, a formal invitation adds a sequence of six. So the eater's obligations grow strictly with the number of people involved, and never shrink. Which means the hardest case is not the solitary meal but the arranged one, where every layer is in force at once." },
      { title: "Note what it means to start the quarter here", body: "And it matters that the quarter about ordinary life opens on eating rather than on marriage or trade. Eating is what everybody does, several times a day, without deciding to — the most frequent voluntary act a person performs and the one least likely to be examined. Starting here signals what the quarter is for: not the notable episodes of a life, but the parts of it that run on habit and are never inspected because nothing about them seems to call for inspection." },
    ],
    closer: [
  ...
  makeChapter({
    ...
    moves: [
      ...
      { title: "Note the two conditions and why the second is harder", body: "Two conditions are being imposed and they are not equally easy to meet. Lawful in itself is a question about the food — this meat, this bread — and it can be settled by looking at what is in front of you. Wholesome in how it was earned is a question about a transaction, and it requires looking past the food to where the money came from, and past that to where the person paying you got it. The first can be answered at the table. The second cannot be answered there at all." },
      { title: "Follow the argument from the order of the prohibitions", body: "And the argument from ordering is a real argument rather than a flourish. The ban on taking people's property wrongfully is placed before the prohibition of killing — and Ghazali reads the sequence as deliberate, magnifying how serious the forbidden is and how much the lawful is worth. Which is why he can end by calling wholesome food not a refinement but one of the obligations and one of the foundations of religion, in a book otherwise full of instructions about washing your hands." },
    ],
    closer: [
  ...
  makeChapter({
    ...
    moves: [
      ...
      { title: "See what the small items cover", body: "And notice that the rest of this part is mostly not about food at all. How it is laid out, how a person sits down to it, what he intends by it, and being satisfied with what is there rather than wanting something else — three of those four concern the eater's disposition before a mouthful is taken. Which is the quiet claim of the chapter: how a meal goes is largely settled before it starts, by what a person arrived at the table wanting." },
      { title: "Note the pattern of report plus reason", body: "The instruction about washing sets the chapter's method: a report is given, and beside it a plain practical reason — the hand is never clean after a day's work. Neither is asked to carry the instruction alone. That matters, because it means a reader who finds one of the reports weakly attested still has a reason in front of him, and a reader who finds the practical reason obvious is being told it is also transmitted." },
    ],
    closer: [
  ...
  makeChapter({
    ...
    moves: [
      ...
      { title: "Note what the eating manners are actually managing", body: "Read the manners of taking food together and they are all about speed. Small mouthfuls, chewed properly, no reaching for the next before the last is swallowed — and he names the fault directly, calling it haste. Nothing here is about quantity or about what is being eaten. What is being slowed down is the mechanism by which a person gets to the end of a meal without having noticed any of it, which is also the mechanism by which he eats more than he meant to." },
      { title: "See why saying the name with each morsel", body: "And the reason given for repeating the name of God at every mouthful is unusually candid: so that greed does not distract him from the remembrance. The instruction concedes that appetite reliably wins over attention across the length of a meal, and rather than exhorting the eater to concentrate, it interrupts him at fixed intervals. It is the same strategy as the varied litanies — designing around a known limit instead of demanding it be overcome." },
      { title: "Take the instruction not to disparage food", body: "And the manner about never faulting food is the one with reach beyond the table. The example is exact: if it pleased him he ate it, and if not he left it — and said nothing either way. Which separates two things people run together, having a preference and voicing it. The preference is allowed and acted on. What is removed is the remark, and with it the small daily habit of assessing aloud what somebody else has provided." },
  ...
  makeChapter({
    ...
    moves: [
      ...
      { title: "Note where the list begins", body: "Stopping before satiety is placed first among the closing manners, ahead of everything practical, and the placement is the argument. It is the only one of them that has to be decided while there is still food and still appetite — every other item here happens after the eating is over and costs nothing. Which makes it the one manner in the chapter that requires something of a person at the moment it applies." },
      { title: "See what the small instructions are protecting", body: "And the run of small items — licking the fingers, wiping them, gathering the crumbs, cleaning the bowl and drinking its water — all point the same way. Nothing is treated as beneath notice and nothing is left. A person who does this cannot regard food as ordinary background material, because he is handling every part of it deliberately. Which is what makes the last instruction possible: seeing the meal as a blessing is not a feeling summoned at the end, it is what the preceding five minutes were building." },
    ],
    closer: [
  ...
  makeChapter({
    ...
    moves: [
      ...
      { title: "See what the seven have in common", body: "The five things the seven govern — who begins, how much is taken, how fast, what is looked at, what is said — are all quantities that were free when the person was alone. Eating quickly alone harms nobody. Taking the largest piece alone is not taking it from anybody. Each of the seven is the same act constrained by there now being somebody else with a claim on the same table, which is why nothing about the food or the eater's own state is repeated here." },
      { title: "Take the first manner as the model", body: "And the first shows how the constraint works: do not begin while somebody present has the better claim to begin — by age, by learning, or by having provided the food. Three different grounds, none of them negotiable in the moment, all of them requiring the eater to have noticed who is at the table before reaching for anything. It is a small deference and it is also a piece of attention, and the six that follow are variations on the same demand." },
    ],
    closer: [
  ...
  makeChapter({
    ...
    moves: [
      ...
      { title: "Note how different this case is", body: "It is worth being clear why this needs its own chapter. Nothing was arranged: somebody has arrived, he is in the house now, and the food is whatever the household happens to have. That is a different situation from an invitation, and it is different in the one way that matters — the host has had no chance to prepare, and so has something to be embarrassed about. Every manner in the chapter is aimed at that embarrassment." },
      { title: "See what the instruction actually costs", body: "And the governing manner is harder than it sounds: bring what you have, straight away, and do not apologise for it. No delay while something better is made, no explaining what you would have provided with more notice. The apology is the natural move and it is the one forbidden, because it converts a visit into an occasion the host is failing at — and the visit, not the meal, is the thing the chapter is protecting." },
    ],
    closer: [
  ...
  makeChapter({
    ...
    moves: [
      ...
      { title: "Note how the duties are distributed", body: "Sorting the six moments by whose duties they are is the useful part of the scheme. Two belong to the host — issuing the invitation and putting the food out. Three belong to the guest — accepting, coming, and leaving. One belongs to both. Which means a guest carries more of the sequence than the host does, and that the common assumption that hospitality is something one person performs and another receives is wrong about most of the occasion." },
      { title: "See why departure gets its own manners", body: "And giving the departure a place of its own says the occasion is not over when the eating stops. There is a way of leaving — when, how, what is said — and it is owed rather than optional. A guest who eats attentively and then goes badly has failed at a sixth of what being a guest consists of, which is a considerably more demanding account of being a guest than most people carry." },
  ...
  makeChapter({
    ...
    moves: [
      ...
      { title: "Follow the chain", body: "The chain is three steps and each is short. Elaborate preparation costs more than it looks — the money, the effort, the disruption. Cost turns into resentment of whoever it was spent on. And resentment of a guest is called resentment of God, which is answered in kind. So a host who overreaches ends up in the worst position in the book, having arrived there by trying to do the thing well." },
      { title: "Note what the instruction is actually guarding", body: "And what is being protected is neither the host's money nor the guest's comfort. It is the host's own feeling toward the person sitting in his house — the one thing the effort of hosting reliably damages and the one thing nobody thinks to guard. Which is why the instruction is stated as a limit on effort rather than as an encouragement to generosity, and why it is the sharpest warning in a book otherwise concerned with crumbs and hand-washing." },
    ],
    closer: [
  ...
  makeChapter({
    ...
    moves: [
      ...
      { title: "Note where the weight actually falls", body: "In a book of small practical instructions, exactly two things are treated as heavy, and it is worth seeing what they are. One is a condition on the food — that it be wholesome in how it was earned, which is called an obligation and a foundation of religion. The other is a warning to a host about his own heart. Neither concerns table manners at all. Everything in between is given plainly, with reasons, and without being made to carry more than it can." },
      { title: "See what the two recurring themes set up", body: "And the two themes running through all four chapters are the ones the rest of the quarter is built on. What is present is enough — which will govern marriage, earning, and travel in turn. And the presence of another person is what generates the additional duties — which is the principle the books on companionship and on enjoining right take up directly. This book is where both are established, on the least consequential material available, before they are applied to anything difficult." },
    ],
    closer: [
