...
export const book29Chapters: Chapter[] = [
  makeChapter({
    ...
    moves: [
      ...
      { title: "Take the measure seriously", body: "A mustard seed's weight in the heart is the disqualifying quantity, and that is not rhetorical exaggeration — it is the reason pride cannot be managed the way most faults are managed. With anger or appetite, a reduced amount is a real improvement and the mean is the target. Here there is no permitted residue, which is why the treatment has to aim at uprooting rather than at proportion, and why the book divides into a diagnosis and two full stations of cure." },
      { title: "Note what the cloak image claims", body: "And the framing report makes pride a claim rather than a manner. Greatness is a cloak and majesty a waist-wrapper, and whoever contests them is broken — which means the proud man is not merely behaving badly toward other people, he is asserting something about himself that belongs to somebody else. Everything harsh in the chapter follows from that, and it is why the fifth section can rank pride toward people as a contest over the kingship itself." },
    ],
    closer: [
  ...
  makeChapter({
    ...
    moves: [
      ...
      { title: "Note how the corrections are made", body: "None of the three men quotes a rule at the strutting man. Hasan, Tawus and Mutarrif each interrupt him with a reminder of what a body is — where it came from, what it contains, what it turns into. Which is the right instrument for this fault, because pride is not a mistaken belief that could be corrected by information; it is an inflation, and what deflates it is being made to see the thing being inflated." },
      { title: "Take Umar's response as the model", body: "And the anecdote rewards the man who takes the observation rather than defending himself. Umar b. Abd al-Aziz does not explain the walk or dispute the characterisation — he admits he learned it limb by limb and drops it. Which is the only useful response available, since any defence of a gait is a further exercise of the thing being pointed at." },
    ],
    closer: [
  ...
  makeChapter({
    ...
    moves: [
      ...
      { title: "Note the shape of the exchange", body: "The central saying makes humility a transaction that runs the wrong way by ordinary reckoning: pardon adds honour rather than diminishing it, and nobody lowers himself for God without being raised. Which is not a promise of eventual compensation but a claim about how the thing actually works — the lowering is the mechanism of the raising, not a price paid for it." },
      { title: "See what the illustrations have in common", body: "And the examples all turn on approach rather than withdrawal, which corrects the natural picture of humility as self-effacement. Seating the afflicted beside you, eating with somebody ill, taking a servant's hand and going wherever she wants to go — every one of these is a movement toward a person that other people are moving away from. Humility here is a direction of travel, not a quantity of self-regard." },
    ],
    closer: [
  ...
  makeChapter({
    ...
    moves: [
      ...
      { title: "Take the three beliefs seriously", body: "The anatomy is exact and it explains why pride is so hard to catch. Three things are required: a level assigned to yourself, a level assigned to somebody else, and yours placed above theirs. Thinking well of yourself is not enough on its own, and neither is looking down on somebody. Which means a person can find each ingredient in himself, dismiss each as harmless, and hold all three." },
      { title: "Note what the disposition actually is", body: "And the disposition is not the beliefs but what sits on top of them — an elation, a reliance, a swelling. Which is why the fault can persist after a man has corrected his opinions about himself and about the other person. The beliefs are what produce it and the state is what has to go, and states do not respond to being argued with." },
      { title: "See how this organises the book", body: "And the distinction from conceit is what splits the book in two. Pride requires somebody to be proud toward; conceit does not, and a person entirely alone could still have it. So conceit is the deeper of the two and gets the second half — which turns out to be right, since the seventh section will name conceit as the single inward source from which pride grows." },
    ],
    closer: [
  ...
  makeChapter({
    ...
    moves: [
      ...
      { title: "Follow the image of the boy on the throne", body: "The image is chosen for its absurdity and it does precise work. A boy putting the king's cap on and sitting on his throne is not committing a crime against another subject — he is claiming the position itself, which is why the offence is of a different order from ordinary defiance. Contesting a king over some of his servants and contesting him over the kingship are not the same act, and pride is being classed with the second." },
      { title: "Take the point about disputing to win", body: "And the practical consequence is stated where a reader would least expect it: pride leads to disobeying the command, because a proud man cannot take truth that arrives through somebody he ranks below himself. Which puts the disputants of the book on knowledge in this chapter, and makes Iblis the exact model — pride toward a creature, on a ground of origin, that ended in refusing an instruction from God." },
    ],
    closer: [
  ...
  makeChapter({
    ...
    moves: [
      ...
      { title: "Note why knowledge is the fastest route", body: "The description of the learned man is uncomfortably specific: he expects to be greeted first, treats an ordinary courtesy as a debt owed to him, and looks at ordinary people as though they were cattle — while about the hereafter he fears for them more than for himself and hopes for himself more than for them. That last clause is the diagnosis. He has taken his learning as a guarantee, which is precisely the thing learning was supposed to make impossible." },
      { title: "Follow the two explanations offered", body: "And two reasons are given, and both are worth holding. Either what is being pursued is not knowledge at all but a craft — since real knowledge is what makes a man know his Lord, himself, and the danger of how he will end. Or the study began before the soul was cleaned. Wahb's image covers both: rain is sweet and clear, and each tree turns it into its own taste, so the bitter tree grows bitterer." },
      { title: "Take the three depths as a diagnostic", body: "And the three degrees are a scale a person can locate himself on. Rooted in the heart with the branches cut, so the conduct is genuinely humble. Showing in behaviour — taking precedence, the averted cheek, the furrowed brow. And showing on the tongue as boasting and competitive display. The first is the most dangerous to be in, because it is the only one at which a man can read his own conduct as evidence that he is clear." },
    ],
    closer: [
  ...
  makeChapter({
    ...
    moves: [
      ...
      { title: "Note the single inward source", body: "The structure here is economical and it matters: inward pride has exactly one cause, which is conceit. A man pleased with himself, or with his knowledge, or with his works, magnifies himself and becomes proud. Which means the whole of the first half of this book is downstream of the second half, and the treatment of pride cannot be completed without the treatment of conceit." },
      { title: "See how rancour and envy produce it", body: "And the outward causes include two that are not about self-regard at all. A man may know perfectly well that somebody deserves deference and be unable to give it, because anger has left a settled dislike — that is rancour doing the work. Envy does the same with no injury ever having occurred. In both cases the refusal to take truth or counsel from that quarter looks like pride and is produced by something else entirely." },
      { title: "Take the solitude test", body: "And the test that closes the section is the most useful thing in the book. Ostentation produces a pride that vanishes in private. Conceit, rancour and envy produce one that persists when the man is alone with the person he looks down on. So what belongs to the crowd is display, and what survives solitude is pride — which gives a person a way of sorting his own case without having to interrogate his motives." },
    ],
    closer: [
  ...
  makeChapter({
    ...
    moves: [
      ...
      { title: "Note how ordinary the occasions are", body: "The list of occasions is deliberately unremarkable: liking it when people stand for you, not making a visit that would help somebody, not letting a man sit beside you rather than in front of you, not walking unless others are behind, doing no work with your own hands, not carrying your own shopping. None of these is a sin and all of them are the daily places where a claim about rank gets made and never examined." },
      { title: "Take the two counter-examples", body: "And the counter-examples are chosen because both men had every reason to be exempt. Sufyan al-Thawri comes when he is summoned, so that his humility gets tested where it can be seen. Umar b. Abd al-Aziz fills his own lamp rather than wake a sleeping servant, and remarks that he went as Umar and came back as Umar, with nothing lost. That last line is the argument: the thing a person is protecting by not doing such work does not exist." },
      { title: "Follow the dress problem to its solution", body: "And the dress question is settled by a test rather than by a rule, which it has to be, since Hasan's remark closes the obvious escape — the wearer of wool can be prouder than the wearer of embroidered silk. What decides it is solitude again: the proud man wants to look well when he is seen and does not care when alone, while a man who simply loves beauty loves it everywhere, in private, even in his cat." },
    ],
    closer: [
  ...
  makeChapter({
    ...
    moves: [
      ...
      { title: "Follow the three stages of the meditation", body: "The meditation runs one Quranic passage through a life: a man who was not a thing worth mentioning, then made from a despised drop, then made to die and be buried, then raised when God wills. Beginning, middle, end. And the point of the structure is that the two ends are unanswerable — nobody can claim credit for where he came from or authority over where he is going." },
      { title: "Take the middle, which is the hard part", body: "And the middle is where the argument actually bites, because it concerns a man at the height of his powers. He hungers without choosing to, sickens without choosing to, dies without choosing to. He wants to know something and cannot; wants to remember and forgets; wants to forget and cannot. The conclusion is stated flatly — he does not own his own heart. Which is a description of the present, not of the beginning or the end, and it is available to anybody willing to look." },
      { title: "Note the drill Ghazali prescribes", body: "And the practical rule is general and usable: look at what pride demands of you, and persist in the opposite until humility becomes a disposition. Then prayer itself is offered as the exercise — standing, bowing, prostrating are exactly the acts humility consists of, and they were prescribed to a people who would not bend down to pick up a dropped whip. The form of the prayer is being read as a treatment for this specific disease." },
    ],
    closer: [
  ...
  makeChapter({
    ...
    moves: [
      ...
      { title: "Note how the external grounds are dissolved", body: "The five worldly grounds are all handled by one observation: they are outside the man. A person proud of his wealth is proud of his horse and his house, and would be lowly again the day the horse died — so the pride was never about him, and the thing it rests on can be removed without his participation. Which is why these take less work than the two religious grounds, where what is being claimed is genuinely his." },
      { title: "Take the reading given for every encounter", body: "The scholar is given a practical way to interrupt comparison. The unlearned person may be more excusable because he acted without knowledge. The learned person knows things the observer does not; the older person began worship earlier; and the younger person has had less time to accumulate wrongs. Even a person currently in error may end in guidance while the observer ends in that person's present condition. Every encounter therefore contains a reason not to claim superiority." },
      { title: "Follow the hardest case", body: "And the hardest case is anger at open wrongdoing held together with humility toward the wrongdoer, which requires three things at once: your own past sins, so your worth shrinks in your own estimation; the recognition that your knowledge and sound belief were given rather than achieved; and honest uncertainty about your own ending. The image is exact — a king's servant charged with disciplining the king's son, genuinely angry and entirely clear that the boy outranks him." },
    ],
    closer: [
  ...
  makeChapter({
    ...
    moves: [
      ...
      { title: "Note that the deficiency is a real fault", body: "Naming a failure on the low side, and giving it a concrete example, is what keeps this from being a book that can only be failed in one direction. A scholar who gives up his seat to a cobbler, straightens the man's sandals and runs behind him to the door has gone past humility into abasement — and Ghazali says plainly that this is not praiseworthy either. Which means humility is a mean, subject to the same double failure as courage and temperance." },
      { title: "Take the test of ease", body: "And the test for whether the disposition is real is behavioural rather than introspective: the act should come easily, without heaviness and without having to be decided on. If a man is finding it heavy while he performs it, he is forcing himself and the training is not finished. That is the same criterion the book on character gave for any settled trait, and it means outward compliance is evidence of progress rather than of arrival." },
    ],
    closer: [
  ...
  makeChapter({
    ...
    moves: [
      ...
      { title: "Report the pairing with despair", body: "Ibn Masud named despair and conceit as two routes to ruin. Ghazali explains why: lasting success requires striving and seeking. The person in despair does not strive, while the self-satisfied person believes he has already succeeded." },
      ...
      { title: "Note the pairing with despair", body: "Ibn Mas'ud's pairing is the sharpest diagnostic in the section, and Ghazali supplies the reason: what is wanted is reached only by striving and seeking, and the despairing man does not strive while the conceited man believes he has already arrived. Two opposite errors producing the same outcome — which explains why conceit is ranked among the destroyers rather than among the unattractive habits." },
      { title: "Take the observation about error", body: "And the point made in passing is the one the book ends on. A man can be just as conceited about a deed in which he is entirely mistaken as about one in which he is right — because the pleasure attaches to his sense of having done well, and that sense does not check whether the deed was sound. Which is why the eighth kind of conceit is treated last and called the hardest of all." },
    ],
    closer: [
    ...
    model: pair("Why the two are named together", "Both remove the striving on which lasting success depends.", [["Despair", "Nothing I do will arrive, so there is no reason to begin.", "warning"], ["Conceit", "I have already arrived, so there is no reason to continue.", "warning"]]),
  }),
  makeChapter({
    ...
    moves: [
      ...
      { title: "Follow the damage to the accounting", body: "The second effect is the most consequential: conceit disables a man's reckoning of his own sins. Some he does not recall or examine at all, supposing there is no need; those he does recall he makes small rather than large, so no serious repair is attempted. Which means the fault does not merely coexist with other faults — it removes the instrument by which any of them would be found." },
      { title: "Note the argument about defective works", body: "And the same happens to the works themselves. He magnifies them, exults in them, and mentions them to God as a favour done, forgetting who enabled them — and once he is pleased with them he cannot see what is wrong with them. Then the conclusion, which is severe: a man who does not inspect the defects in his works has most of his effort wasted. The acts still happened; the inspection that would have made them worth something did not." },
      { title: "See why it closes off learning", body: "And the last effect reaches outward. Conceit in one's opinion or intellect stops a person consulting, stops him asking, and makes him disdain to question somebody who knows more. Which is self-sealing: the man is now cut off from the correction that would reveal the fault, and the fault is what cut him off. It is the same structure as the eighth kind at the end of the book." },
    ],
    closer: [
  ...
  makeChapter({
    ...
    moves: [
      ...
      { title: "Note that the perfection is granted", body: "The analysis begins by conceding that the quality is real. Conceit only ever concerns something that genuinely is a perfection — so none of what follows depends on the man being wrong about what he has. Which is what makes the section useful, since a treatment that worked by deflating people's estimate of their own qualities would be useless against anybody whose estimate was accurate." },
      { title: "Follow the three states", body: "And the three states are separated by one variable: what the man does with the awareness that it was given. Fearing its loss is not conceit. Rejoicing in it as a blessing from God is not conceit. Conceit is the third — not fearing, rejoicing, and resting secure, with the joy taken in it as his own achievement. So the same quality, the same pleasure, and the difference is entirely in whom the quality is taken to belong to." },
      { title: "Take the definition of presumption", body: "And presumption is defined as the stage past that, where a man comes to hold a claim on God: expecting honour for his work, and finding it less likely that anything unpleasant should reach him than that it should reach the corrupt. Which is a considerably more specific fault than general complacency, and it is testable — a man who finds himself surprised by misfortune, in the sense of finding it unfair, has arrived there." },
    ],
    closer: [
  ...
  makeChapter({
    ...
    moves: [
      ...
      { title: "Follow the dilemma", body: "The argument is a dilemma and both horns close. Ask what you are pleased with yourself for: being the place the act happened, or being what produced it. If the place — a receptacle is acted upon and contributes nothing to bringing anything about, so there is nothing there to take pleasure in. If the producer — then the power, the will, the limbs and every condition that made it possible have to be accounted for, and none of them was yours before it was given." },
      { title: "Take the closing of the regress", body: "And the regress is closed with the sharpest image in the book. You say the second gift came because of a quality you already had — but that quality was also given. He gave you the horse and then the servant; and if you say the servant came because you owned a horse, the horse was given too. And if you say the worship was granted because you loved Him, then who put the love in your heart. There is no step at which the chain reaches something the man supplied." },
      { title: "Note the two answers on reward", body: "And the objection about reward — how anything can be deserved if the acts are God's creation — gets two answers rather than one. A strict answer, that the man and his power and his movement are all created, with the full treatment deferred to the book on gratitude. And a concessive answer that does not require the strict one. Which means the argument against conceit does not depend on winning the theological dispute about human action." },
    ],
    closer: [
      { title: "The treasury and the keys", body: "Imagine all the world's wealth inside a fortified building, with its key in a treasurer's hand. Sitting at the gate for a thousand years would not bring out a single coin. Once the key is given, the wealth can be reached by extending a hand. The remarkable part is receiving the key, not moving the arm. In Ghazali's analogy, acts of worship lead to lasting success, while their keys — power, will, and knowledge — are in God's hand." },
  ...
  makeChapter({
    ...
    moves: [
      ...
      { title: "Note how the complaint is dismantled", body: "The complaint is the intelligent poor man looking at the heedless rich man and coming close to seeing injustice. And the answer does not deny the disparity — it points out that giving both to the same man would leave the poor and ignorant man with the identical complaint and a stronger case. Which shows the grievance is not about a principle at all, since the principle would have to be abandoned the moment it was satisfied in his favour." },
      { title: "Take the exchange test", body: "And then the test that settles it, which is a question rather than an argument: would you take the other man's ignorance along with his wealth, in exchange for your intelligence and your poverty? The refusal is immediate, and the refusal is an admission — the man has just judged his own portion the greater one. Ali's line supplies the principle behind it: a man's intellect is counted as part of what he was given." },
      { title: "See the shape of the argument", body: "And the shape is the horse and the servant again, now used against a grievance rather than against conceit. A man given a horse asks the king why he was not also given the servant — on the ground that he owns a horse. The blessing has been converted into a claim on the next blessing, which is the exact operation the earlier section identified, running here in the register of complaint instead of pride." },
    ],
    closer: [
      { title: "David and Hunayn", body: "David said that every night included someone from his household in worship. The response asked where that strength had come from and reminded him that it could not exist without divine help. Ibn Abbas read David's fall as beginning when he took satisfaction in the work and attributed it to his household. By asking to be tested, he presumed upon patience before the trial arrived. At Hunayn, the believers similarly relied on their strength and numbers." },
  ...
  makeChapter({
    ...
    moves: [
      ...
      { title: "Note the pattern in the treatments", body: "Each of the first several treatments works the same way: name how fast the thing can go. A fever dissolves strength past recovery; a slight disorder of the brain deranges the mind; children, servants, clan and supporters all separate from you at the graveside. The method is not to argue that these are worthless but to point out how thin the tenure is — which is enough, since conceit consists in resting secure." },
      { title: "Take the treatment for intellect", body: "And the treatment for conceit in one's own intellect is the most practical instruction in the book. Measure it from other people rather than from yourself, and from your adversaries rather than your friends — since a flatterer's praise increases the conceit and tells you nothing. Then look at fools who are delighted with their own intelligence while everybody laughs at them, and fear being one of them without knowing it. The point is that the faculty being assessed is the faculty doing the assessing." },
      { title: "Follow why the eighth is the hardest", body: "And the last kind is conceit in a mistaken opinion, which Ghazali calls harder to treat than all the rest — and the reason is structural rather than moral. The man does not know he is wrong, and would drop the view if he did. So the disease consists in not recognising the disease, and there is no route by which the information reaches him. Ending a book on pride and conceit here is exact: the final form of the fault is one nobody can be argued out of." },
    ],
    closer: [
      ...
      { title: "And the counsel for everyone else", body: "Ghazali gives different advice to a person who will not devote his life to this field of knowledge: do not plunge into the competing schools or attempt to judge all their arguments. Hold that God is one, has no partner, and that nothing is like Him. Accept the Messenger as truthful, follow the way of the early community, and believe what came in the Book and the prophetic practice without probing for technical detail. Give your attention to God-consciousness, avoiding disobedience, fulfilling obligations, and showing compassion toward Muslims." },
    ],
...
export const book29ConceptNodes: ConceptNode[] = [
  ...
  ["despair", "Despair", "The opposite estimate", "Named beside conceit because both end the striving on which lasting success depends."],
