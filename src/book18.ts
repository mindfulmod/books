...
export const book18Chapters: Chapter[] = [
  makeChapter({
    ...
    moves: [
      ...
      { title: "Note the order of the first chapter", body: "The order of business is unusual and it is the most telling thing about the book. The opposing schools are transmitted first, at length, from named founders; then the evidence for permission; then the answers to what the prohibitors relied on. Which means a reader meets the strongest case against Ghazali's position before he meets a word of the position itself — and if he stops reading a third of the way in, he leaves holding the view Ghazali is going to argue against." },
      { title: "See what the second chapter changes", body: "And the second chapter shifts the question from whether to what, which is where the useful material is. Once the four states of the listener are set out, the argument about permissibility looks different: it stops being a question about a practice and becomes a question about a person, and the same gathering turns out to contain cases falling on both sides of the line at once." },
    ],
    closer: [
  ...
  makeChapter({
    ...
    moves: [
      ...
      { title: "Note who is on the prohibiting side", body: "The names matter and the book puts them first for that reason: al-Shafi'i, Malik, Abu Hanifa, Sufyan, and a group of the scholars. That is not a fringe position being cleared out of the way — it is the founders of the schools, and a reader who arrives certain the practice is fine is being shown that his certainty is one side of a live disagreement among the people whose rulings he otherwise follows." },
      { title: "Take al-Shafi'i's wording seriously", body: "And the words are not softened in transmission. Singing is called a frivolity, disliked, resembling the false, and a man who does much of it is a fool whose testimony is rejected — which is a legal consequence, not a term of abuse. Ghazali gives it in that form, with the further restriction from al-Qadi Abu al-Tayyib about listening to a woman not closely related, permitted neither unveiled nor from behind a screen. He is not building a straw case to knock down." },
    ],
    closer: [
  ...
  makeChapter({
    ...
    moves: [
      ...
      { title: "See what separating the three links does", body: "The chain is the book's central device and everything follows from it. Hearing produces a state in the heart; the state produces movement in the body. Three things, in sequence, each capable of a separate ruling — which means the question everybody argues about is really three questions that have been running together. Somebody objecting to what he saw at a gathering is usually objecting at the third link and stating his objection as though it settled the first." },
      { title: "Note what the chain also implies", body: "And the chain runs one way, which has a consequence people miss. Nothing at the third link can be assessed without knowing what happened at the second, and nothing at the second without the first. So the visible part — the movement, the crying out, the torn clothing — is the least interpretable thing in the sequence, despite being the only part an observer has access to. The device is built to make that mismatch impossible to ignore." },
    ],
    closer: [
  ...
  makeChapter({
    ...
    moves: [
      ...
      { title: "Note where the variable sits", body: "The first station is understanding, and understanding differs by the state of the listener — which is where the whole practical half of the book is decided. Not by what is sung, not by who is present, not by what the gathering looks like. By what the person listening is in a position to hear, which is a fact about him that he brings with him and that nothing about the occasion changes." },
      { title: "See what having four states rather than two accomplishes", body: "And four states rather than a simple permitted and forbidden is what makes the analysis usable. Two categories would force every case to one side, and the arguments about audition are so intractable precisely because both sides can point to real instances. Four means the practice is not one thing — the same sound reaches a beast's pleasure, an appetite, an aspirant's condition, and something past all of them, and the ruling has to follow the case rather than the sound." },
    ],
    closer: [
  ...
  makeChapter({
    ...
    moves: [
      ...
      { title: "Note how the first state is ranked", body: "The first state is permitted and it is also called the meanest rank, and both halves are worth holding. Enjoying a melody for its own sake breaks no rule and requires no defence. It also requires nothing but being alive — the camel shares it, and so does every animal with a kind of pleasure in good sounds. So the verdict is permission without any credit attached, which is a distinction most arguments about music never manage to draw." },
      { title: "See what the ranking is doing to the debate", body: "And that single move disarms both sides of the usual quarrel. To the person defending the practice: your pleasure in the sound is real, allowed, and establishes nothing whatever about the spiritual worth of what you are doing. To the person attacking it: what you are attacking, at this level, is something animals do, which is not the sort of thing prohibition is aimed at. Neither party gets what they came for." },
    ],
    closer: [
  ...
  makeChapter({
    ...
    moves: [
      ...
      { title: "Note what the second state actually is", body: "The second state is understanding put to work on a created object — a particular person, or an unspecified one — and Ghazali names whose it is without euphemism: the young, and people governed by appetite, applying what they hear according to what they want. Which is a precise description of how most listening to love poetry has always functioned, and it is placed second so that nobody can mistake the four states for four grades of piety." },
      { title: "See what the refusal to analyse says", body: "And then he declines to treat it: too base to speak about except to call it base and forbid it. In a book that gives the first state a rank and a reason, the third a mechanism, and the fourth an extended image, the absence of any analysis here is itself the ruling. There is nothing to work out, no conditions under which it improves, and no treatment offered — which is a harsher verdict than a paragraph of argument would have been." },
    ],
    closer: [
  ...
  makeChapter({
    ...
    moves: [
      ...
      { title: "Follow why the third state works", body: "The mechanism given is precise and it explains why the same words do nothing for one person and a great deal for another. An aspirant already has an object he is directed toward, and what he hears finds something in him aimed that way and attaches to it. Nothing is being supplied by the sound; the sound is being taken up by a direction that was already there. Which is why the practice cannot manufacture a state in somebody who has none." },
      { title: "Take the two poles seriously", body: "And what he applies it to is not a settled condition but an alternation — enablement at one time and hindrance at another. That is the honest description of an aspirant's life, and it explains something practitioners notice and rarely account for: the same gathering, the same verses, and one week they open everything and the next they are just words. The variable is not the occasion. It is which pole the listener is at when he arrives." },
    ],
    closer: [
  ...
  makeChapter({
    ...
    moves: [
      ...
      { title: "Follow the second passing-away", body: "The fourth state has two stages and the second is the harder one. First understanding of anything but God leaves him, and then he leaves himself — his own states and his dealings with them. Then the further step: he passes away from the witnessing as well, because a heart that turns toward its own witnessing, and toward itself as witnessing, has at that moment become heedless of what it was witnessing. The self reappears the instant it notices what it is doing." },
      { title: "Note the argument from the drunk", body: "And the argument offered for it is ordinary rather than mystical, which is what makes it land. A man absorbed in something has no attention left for his seeing, or for the eye he sees with, or for the heart taking the pleasure. The drunk gives no account of his drunkenness and the man enjoying something gives no account of his enjoyment — the only thing he can report on is the thing itself. Which means any report of the fourth state, offered by the person in it, is evidence he has left it." },
      { title: "See why the women cutting their hands is the right image", body: "And the image is exactly chosen. The women at the sight of Joseph did not decide to cut their hands and did not notice doing it; sensation itself dropped away under what they were looking at. That is a case of absorption everybody grants, from a source nobody disputes, involving no claim about spiritual attainment — which lets Ghazali describe the structure of the state without asking the reader to accept anything about who reaches it." },
    ],
    closer: [
  ...
  makeChapter({
    ...
    moves: [
      ...
      { title: "Take Dhu al-Nun's sentence as the book in miniature", body: "The sentence is the whole argument compressed. One incoming, described as true, unsettling hearts toward the Real. And then the split: whoever inclines to it by the Real is verified, whoever inclines to it by the self becomes a heretic. Same sound, same gathering, same moment — and opposite outcomes, decided entirely by what each listener brought. Ghazali could have written that himself; he has an authority say it, which puts the position inside the tradition rather than alongside it." },
      { title: "Note how he handles the discussion of ecstasy", body: "And notice the procedure at this link: he reports that there is long discussion among the Sufis and among those who examine how audition suits the spirits, transmits what they said, and then gives what he takes the reality to be. It is the same method as the first chapter — the existing positions in full before his own — and it is why this book, on the most contested subject in the quarter, reads less like advocacy than anything around it." },
    ],
    closer: [
  ...
  makeChapter({
    ...
    moves: [
      ...
      { title: "Note why the movement is treated last", body: "Movement is the third link and it is placed last for a structural reason, not a rhetorical one. It is produced by ecstasy, which is produced by understanding, which depends on the listener's state — so nothing here can be assessed on its own. A man crying out or tearing his clothes is displaying the terminal effect of a chain whose first two links are invisible, and the same display can sit at the end of two entirely different chains." },
      { title: "See what the measured and unmeasured distinction is for", body: "And the division between unmeasured movement, called agitation, and measured movement, called clapping and dancing, is drawn at the very head of the book rather than here — which suggests how much weight it carries. Measured movement is patterned, which means it is being produced with some part of the person attending to how it looks. Unmeasured movement is not. The distinction gives an observer the one thing at this link he can actually assess." },
    ],
    closer: [
  ...
  makeChapter({
    ...
    moves: [
      ...
      { title: "Note the cost of the position", body: "Ghazali's conclusion carries a price he does not hide: if the ruling turns on the state of the listener, then nobody but the listener can apply it. Two men at the same gathering, hearing the same verses, may be in the second state and the third, and nothing observable separates them — not their faces, not their movements, not what they say afterwards. Which makes the doctrine correct and largely unenforceable, and he lets that stand rather than trading it for a rule about the practice." },
      { title: "See what the book refuses to claim", body: "And the book ends without claiming to have closed the disagreement. He argues a position and answers the prohibitors' evidence, but the founders of the schools are still on the record where he put them in the second section, and nothing in the four states dislodges them. Which is consistent with how he opened: the strongest case against him, transmitted first and at length, is still standing at the end — as a live disagreement rather than as a difficulty he disposed of." },
    ],
    closer: [
