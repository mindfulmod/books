...
export const book17Chapters: Chapter[] = [
  makeChapter({
    ...
    moves: [
      ...
      { title: "Note what the taxonomy is for", body: "The book's real content is a classification of journeys, and it is worth seeing why one is needed. Travel has no form of its own — it is not an act with elements and conditions like prayer, and nothing about the movement distinguishes a pilgrim from a merchant from a man escaping a quarrel. So there is nothing to legislate until the journeys have been sorted by what they are for, and the sorting is what lets the rest of the book say anything at all." },
      { title: "See why the technical chapter is short", body: "And the second chapter is brief for a reason that says something about the whole subject. What a traveller specifically has to know is small: two dispensations in purification, two in prayer, how to find the direction, how to judge the times. Everything else that happens on a journey is governed by rules he already had — the manners of eating, the rights of companions, the duties of a Muslim. Travel does not create a new body of law. It puts an existing one under strain." },
    ],
    closer: [
  ...
  makeChapter({
    ...
    moves: [
      ...
      { title: "Note where the first cut is made", body: "The first division is made by where the motive is, which is an unusual place to cut. Not by whether the journey is worthy, or religious, or necessary — but by whether the thing driving it lies behind the traveller or ahead of him. Flight and seeking. Which means two journeys along the same road, at the same speed, to the same place, can fall on opposite sides of the division, and the whole taxonomy follows from a fact nobody watching could observe." },
      { title: "See what the definition of flight concedes", body: "And the definition of flight is candid in a way that is easy to miss: he has something unsettling him where he is, and but for it he would have nowhere to go. So the destination is incidental — it is not that anywhere in particular was wanted, only that here became impossible. Ghazali states that plainly rather than dressing it as a journey toward something, and the honesty is what lets the next two sections list unglamorous reasons without embarrassment." },
    ],
    closer: [
  ...
  makeChapter({
    ...
    moves: [
      ...
      { title: "Note what is on the list and what is not said about it", body: "Plague, epidemic, a fear arising from strife or a quarrel, a rise in prices. Read the list and notice what is absent: any suggestion that leaving for these reasons is second-best, or that a stronger person would stay. Disease, disorder, a private quarrel and the cost of living are set down as reasons for travel and the book moves on. In a work that scrutinises motives as closely as the Ihya does, the absence of scrutiny here is itself a position." },
      { title: "See why the general and particular split matters", body: "And the division between general and particular is more than a tidy pair. A general affliction — the plague in a town, prices across a market — leaves nothing for the sufferer to answer for; anybody there is in the same position. A particular one means somebody has been singled out, and that raises a question the general case does not: why him. Ghazali does not pursue it, and by putting the two side by side he declines to make leaving harder for the person who was targeted." },
    ],
    closer: [
  ...
  makeChapter({
    ...
    moves: [
      ...
      { title: "Take the vocabulary seriously", body: "The word is the thing to catch: a man afflicted in his town with status and wealth. Not blessed with them, not tested by them — afflicted, in the same register a disease would be. That is the vocabulary of the books on the world and on wealth in the Quarter of Perils, and finding it here, in a practical chapter about reasons to travel, shows how thoroughly the position runs through the work rather than being confined to the books that argue for it." },
      { title: "Note what the second example concedes", body: "And the second example is a case where leaving is the only remedy available: a man being pushed toward an innovation, or into a position he has no right to hold. He cannot argue his way out — the pressure is social and it is applied by people who are not going to stop. So the journey is not a spiritual exercise or a search for anything. It is the physical removal of oneself from a situation that would otherwise win, which is a considerably more practical account of travel than the category usually gets." },
    ],
    closer: [
  ...
  makeChapter({
    ...
    moves: [
      ...
      { title: "Note the second kind of knowledge", body: "The second is the one that could not be got any other way, and it is worth stating plainly what it is: learning your own character by seeing what you turn into once your usual conditions are removed. At home a person is supported by habits, familiar people and arrangements that keep him roughly the shape he intends. On a road none of that is holding him up, and what appears is information he could not have obtained by reflection." },
      { title: "See what the third kind admits", body: "And the third is the widest thing in the book: knowledge of the signs of the earth and its wonders, with Dhu al-Qarnayn's going about the regions given as the model. That is travel for the sake of seeing what is there, listed among the religious motives without further defence. Which is consistent with the Ihya's general position that the seen world is a stair to the unseen — but it is still striking to find sheer curiosity about the earth counted as a religious reason to set out." },
    ],
    closer: [
  ...
  makeChapter({
    ...
    moves: [
      ...
      { title: "Note the reason given for visiting the living", body: "The reason attached to visiting living scholars is not what one expects. Blessing is mentioned, but so is something plainer: that from looking at their states a person gains strength of desire to imitate them. That is a claim about how motivation actually works — seeing somebody live a certain way does something no description of it can do — and it is the same argument the book on creed made when it listed sitting with the righteous among the four things that root a belief." },
      { title: "See how wide the category of action is", body: "And the range under action is worth registering. Three fixed journeys with their own forms — the greater and lesser pilgrimages and struggle in God's path. Three places, and the frontiers besides. Graves of the dead and the houses of the living. It is a broad category, and what unifies it is not the destination but that in each case the traveller is going toward something rather than away, which is the division the whole taxonomy was built on." },
    ],
    closer: [
  ...
  makeChapter({
    ...
    moves: [
      ...
      { title: "Note where the sequence starts", body: "The manners begin before the traveller has moved, and the items are all about what is being left rather than where he is going: wrongs returned, debts settled, provision made for the people who depend on him. Which treats departure as a settling of accounts. A man who leaves with these outstanding has not merely postponed them — he has put them beyond reach of the people they concern, which is what makes them the first thing the chapter addresses." },
      { title: "See why the will belongs here", body: "And the will belongs at the start for the reason the book on pilgrimage gave when it placed it at the same point: a traveller and his money are both at risk. Writing it is not a piety performed before a journey but an acknowledgement that this particular journey may not have a return leg — the same recognition the pilgrimage chapters built their whole reading of the two plain garments on." },
      { title: "Note when the companion is chosen", body: "And choosing whom to travel with is placed among the things done before setting out, not among the manners of the road. That is deliberate. The first sentence of the book called travel a kind of mixing, and the book on companionship had already established that not everyone is fit for company and that the assessment is made in advance. On the road there is no assessment left to make — you are with whoever you left with." },
    ],
    closer: [
  ...
  makeChapter({
    ...
    moves: [
      ...
      { title: "Note the instruction about the mounts", body: "The instruction that the backs of mounts are not to be used as chairs, and that a rider should get down morning and evening, is a rule about the animal rather than about the traveller — and it is the kind of item that shows what the chapter's register is. Nothing religious is at stake, nobody is watching, and the animal has no claim anybody would enforce. It is placed among the manners because a journey is a set of small dealings with whatever is carrying you, and those are treated as dealings." },
      { title: "See what the road actually tests", body: "And most of the section is about people under strain: how provision is shared, how fast the party goes, who carries what, and how disputes between travellers are settled. That is Book 15's rights of companionship applied under conditions designed to break them — confinement, tiredness, scarcity, and no way for anybody to leave. The eight rights were stated for friendships at rest; here is the same material with every mitigating circumstance removed." },
    ],
    closer: [
  ...
  makeChapter({
    ...
    moves: [
      ...
      { title: "Note what ending at home establishes", body: "Running the sequence to the last return, rather than to the destination, is the same decision the pilgrimage book made and it carries the same claim. A journey is undertaken from a life and has to be brought back to it. End the account at the arrival and everything gained sits at the far end of the road with no provision for carrying it home — which is where, in practice, most of what a journey produced is lost." },
      { title: "See what the manners of return actually cover", body: "And the content is domestic rather than ceremonial: what is said on the approach, how a man comes back to his household, what he brings with him. Which locates the risk precisely. The failure at the end of a journey is not that a traveller forgets what he learned; it is that he arrives badly — full of where he has been, to people who were not there — and the manners are aimed at that first hour rather than at anything reflective." },
    ],
    closer: [
  ...
  makeChapter({
    ...
    moves: [
      ...
      { title: "Note what the two dispensations have in common", body: "Two dispensations in purification and two in prayer, and each of the four removes a condition that ordinary settled life makes easy. Wiping over footgear and dry ablution answer a shortage of water; shortening and combining answer a shortage of time and of stopping-places. None of them lowers what is owed — the prayer is still performed and the purity still obtained — and what changes is the route, which is the ordinary shape of a dispensation." },
      { title: "See why direction and time become skills", body: "And the last two items mark a real change in a traveller's position. At home the direction of prayer is settled by how a building faces and the times are announced from a minaret — both are facts a person receives. On a road neither is supplied, and both must be worked out from the sun, the stars and the land. So travel converts two things that were given into two things that have to be judged, and the traveller who cannot judge them has lost the ability to pray properly rather than merely the convenience of it." },
    ],
    closer: [
