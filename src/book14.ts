...
export const book14Chapters: Chapter[] = [
  ...
  makeChapter({
    ...
    moves: [
      ...
      { title: "State what it establishes", body: "Ghazali's first move is to say what kind of thing this report is: it is a text establishing the three categories. That word matters. It means the middle is not an awkward gap left where two clear rules fail to meet — it is a third thing, named, with a status of its own and rules of its own. Which is why a whole chapter can be about it, and why the question is never simply whether a case falls under the lawful or the unlawful." },
      { title: "Name the difficulty", body: "And then he names the difficulty rather than working around it. The hard one of the three is the middle, the one the report itself says many people do not know, and it is the doubtful. So it has to be set out and the cover lifted from it. Notice he does not treat the report's own admission of difficulty as a reason to leave the subject alone; he treats it as the reason the subject needs a chapter." },
      { title: "Give the hope in the clause", body: "The warrant comes out of the wording, and it is a small piece of close reading with a great deal resting on it. The report says many people do not know them. Ghazali's inference: what many do not know, a few may know. Had it said nobody knows them, the chapter would be impossible and scruple would have no guide but nerves. Because it says many, the doubtful is difficult rather than closed — and the rest of the book is the attempt to be among the few." },
    ],
    closer: [
  ...
  makeChapter({
    ...
    moves: [
      { title: "Give the definition", body: "The absolutely lawful is defined on two axes at once, and both have to come out clean. First, the thing itself: free, in its own substance, of any quality that would require it to be prohibited. Second, its causes: nothing has entered along the way through which a prohibition, or even a dislike, could attach to it. So the definition is not about what a thing is. It is about what a thing is plus everything that has happened to it." },
      { title: "Note the two conditions", body: "That second axis is what makes the category hard, and it is worth seeing why. The substance of a thing you can inspect. Its history you cannot — it reaches back through every hand it has passed through, every transaction, every ownership, and any one of those could have introduced a defect that survives into the thing sitting in front of you. A loaf can be entirely wholesome and still fail the definition on something that happened three owners ago." },
      { title: "Give the example", body: "So the example he reaches for is rain — caught before it has landed on anybody's property, by a man standing there gathering it out of the air, on his own land or on land nobody owns. Read the clauses one at a time and you can see each of them shutting off a route by which a claim might enter. Before it falls: nobody's ground has touched it. Out of the air: no vessel of anyone else's. On his own land or unowned: no trespass in the standing." },
      { title: "Note what the example shows", body: "What is left is the one case with no history at all — and that is the point of choosing it. Ghazali is not offering rainwater as a diet. He is showing you what the absolutely lawful has to look like, so that you can see how few things reach it, and stop expecting ordinary food to. The category is a limiting case, set at one end of a scale, and almost everything anybody eats sits somewhere along that scale rather than at its end." },
    ],
    closer: [
