...
export const book06Chapters: Chapter[] = [
  ...
  makeChapter({
    ...
    moves: [
      ...
      { title: "See why these are three fasts and not three efforts", body: "Look at what changes between the degrees and what does not. The structure is identical each time — an organ, and something it is held back from — and only the terms change. Stomach held from food. Limbs held from sins. Heart held from everything that is not God. That is why they are not levels of intensity: a person doing the first with enormous effort is not thereby doing the second at all, because the second concerns different organs and different objects. You do not reach it by trying harder." },
