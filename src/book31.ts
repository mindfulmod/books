...
export const book31Chapters: Chapter[] = [
  makeChapter({
    ...
    moves: [
      ...
      { title: "Note that repentance is defined as a compound", body: "The definition is the most consequential thing in the book, and it is a compound of three parts rather than a state. Knowledge, then the pain that knowledge produces, then the action that pain produces. Which means repentance is not a feeling that arrives — it has an entry point that a person can actually work on, and the whole treatment at the end of the book depends on that being true." },
      { title: "Take the causal order seriously", body: "And the parts are causally ordered, not merely listed. The knowledge produces the pain; the pain produces the will to act. Which explains why remove the first and you get remorse with no diagnosis — a man sorry about something he has not understood, who will be sorry again next month. And remove the third and you get a feeling that changed nothing, which is the ordinary condition most people mistake for repenting." },
    ],
    closer: [
  ...
  makeChapter({
    ...
    moves: [
      ...
      { title: "Note where the book is placed", body: "This is the first book of the Quarter of Deliverance, arriving directly after ten books of diagnosis — and the placement is an argument. Everything the Quarter of Perils identified is treatable, and repentance is the movement by which any of it can begin to be treated. Which is why the saving qualities open here rather than with patience or trust: nothing else can start until a person has turned round." },
      { title: "Take the note the quarter opens on", body: "And the reports gathered here place the returning servant somewhere the never-fallen would not obviously stand, which is a striking thing to establish at the opening of a quarter about deliverance. It means the material of the previous ten books is not simply a record of what has gone wrong in a person. It is the material out of which something is made — and the return is not a repair of a life but part of its shape." },
    ],
    closer: [
  ...
  makeChapter({
    ...
    moves: [
      ...
      { title: "Follow the answer about commanded feelings", body: "The objection is a real one and the answer is the useful part of the section. Regret looks like a feeling, and feelings cannot be commanded — but regret is produced by knowledge, and knowledge is within reach. So what is being required is not the state directly but the work that reliably yields it: attending to what sins actually are and what they cost. The command lands on something a person can do this afternoon." },
      { title: "See why the third part shrinks so easily", body: "And the third part is the one people declare complete first, because stopping is visible and stopping is what they did. But abandonment carries two more things inside it — the resolve extending to the end of life, and the repair of what can be repaired. A man who has stopped, has made no resolve about the future, and has returned nothing has satisfied the most conspicuous third of one of the three parts." },
    ],
    closer: [
  ...
  makeChapter({
    ...
    moves: [
      ...
      { title: "Note why delay is a separate matter", body: "The argument is short and it closes the obvious escape. If sins stand between a person and everything he loves, then an hour of deferral is an hour of that separation chosen deliberately — which is a different act from the original slip, and one made with full knowledge. Delay is not the absence of repentance; it is something a person is doing." },
      { title: "Take the mechanism of deferral", body: "And the mechanism named here is the one the whole book keeps returning to: the intention is kept alive precisely so that the act need not happen. A man who means to repent later is not neutral — he is persisting, with an intention attached, and the intention is exactly what makes the persistence bearable to him. Which is why the third class in the fourteenth section is described as a soul that makes things fair to itself." },
    ],
    closer: [
  ...
  makeChapter({
    ...
    moves: [
      ...
      { title: "Follow the removal of the two exemptions", body: "The two exemptions removed are the two everybody assumes. Rank does not exempt — and the argument runs the other way, since the more a person knows of God the larger the smallest opposition looks to him. And a good condition does not exempt, which is why the third pillar is about permanence rather than about an occasion. Neither advancement nor a settled state gets anybody out." },
      { title: "See what universality does to the question", body: "And once no one is exempt, the question stops being whether repentance applies and becomes only what it applies to. Which is a genuine redirection of attention: a reader who was assessing whether this book concerns him is now assessing which of his own acts it concerns, and that is what the second pillar is for." },
    ],
    closer: [
  ...
  makeChapter({
    ...
    moves: [
      ...
      { title: "Note where the anxiety gets moved", body: "This is the section's real work and it is worth naming. Every sound repentance is accepted, necessarily — so the uncertainty a person carries is not about what will be done with him, which is unanswerable, but about whether the three parts were actually present, which is answerable. The anxiety is relocated from a question nobody can settle to one a person can examine." },
      { title: "Take the limit on the promise", body: "And the limit is stated carefully: acceptance of the repentance is not the removal of every consequence. What is owed to other people survives it, which is the whole subject of the twelfth section. Which keeps the reassurance from doing the damage it usually does — a man cannot conclude from being forgiven that the accounts with the people he wronged have been closed." },
    ],
    closer: [
  ...
  makeChapter({
    ...
    moves: [
      ...
      { title: "Note why sorting by source rather than severity", body: "The sins are sorted by which of a person's own traits they come out of, and that decision wires this book directly into the anatomy of the heart in Book 21. The reason is practical: each fault is cured by opposing its own cause, so a classification by outward severity tells a person how bad his situation is and nothing about what to do. Sorting by source tells him where the work is." },
      { title: "Take the second distinction seriously", body: "And the second sorting — what belongs to God's right and what belongs to somebody else's — is not a refinement but the distinction the whole third pillar rests on. The two kinds are discharged differently, and a man who has not separated them will treat a debt to a person as though it were settled by turning back to God. Which is exactly what the twelfth section is written to prevent." },
    ],
    closer: [
  ...
  makeChapter({
    ...
    moves: [
      ...
      { title: "Note how the distinction is handled", body: "The division into minor and major is granted rather than dissolved — it is in the sources and the section does not begin by explaining it away. What is then shown is that it is not a fixed list: what makes something major turns out to be sensitive to circumstance, which a table of categories would conceal. So the distinction survives and stops being the sort of thing a person can look up." },
      { title: "Take the opposing view he records", body: "And he records the position of those who deny the distinction altogether — that every opposition is major relative to the majesty of the one being opposed — without dismissing it. Which is characteristic, and it also does work: holding that view in mind is what stops the minor category from becoming a licence, while the distinction itself stops a person from despairing over everything at once." },
    ],
    closer: [
  ...
  makeChapter({
    ...
    moves: [
      ...
      { title: "Note the unusual restraint", body: "This section says more often than any other in the quarter that it does not know. Ghazali follows what the Quran details and stops where it stops, marks parts of the account as supposition rather than certainty, and says outright that certain knowledge of it belongs to the world of prophecy. In a book full of confident diagnosis, that is a deliberate change of register on the one subject where nobody has access." },
      { title: "See what the reader is left with", body: "And what remains usable is a direction rather than an arithmetic: good deeds bear on evil ones, and the weight is affected by the causes the next section lists. Which is enough to act on and not enough to calculate with — and the calculation is precisely what a person would misuse, since knowing the exchange rate would turn repentance into accounting." },
    ],
    closer: [
  ...
  makeChapter({
    ...
    moves: [
      ...
      { title: "Take the image of the drops", body: "The image is exact and it explains why persistence is placed first. The same quantity of water poured at once runs off a stone and leaves nothing; falling drop after drop in the same place, it wears a hollow. So the damage is a function of repetition rather than of amount — which means a man can commit nothing anybody would call serious and still be worn through." },
      { title: "Follow the point about what flanks a great sin", body: "And the observation attached is one people rarely make: a great sin is hardly ever committed suddenly. The adulterer rarely arrives without preliminaries; the killer rarely without a prior enmity. So every great sin has small ones on both sides of it, before and after — which means attending only to the large ones is attending to the one part of the sequence that was never where the decision got made." },
      { title: "Note the two images for deeming it small", body: "And the second cause turns on a mechanism worth stating: magnifying a sin comes out of the heart's aversion to it, and aversion limits what it can do; making it small comes out of familiarity, and familiarity deepens the mark. Then the two images — the believer who sees his sin as a mountain overhead that he fears will fall, and the hypocrite who sees it as a fly that passed his nose and was waved away. The fly is the more dangerous, precisely because nothing about it demands attention." },
    ],
    closer: [
  ...
  makeChapter({
    ...
    moves: [
      ...
      { title: "Note the sign given for regret", body: "Ghazali gives regret an observable mark rather than leaving it as an inward state to be guessed at: tenderness of the heart and abundance of tears. Which is not sentimentality but a test — the condition being described is one that shows, and a man who finds nothing softening in himself has some information about where he actually is." },
      { title: "Follow the answer about bitterness", body: "And the objection is a fair one: sins are things nature wants, so how does anybody find them bitter. The answer runs through a corrupted appetite — the palate that recoils from wholesome food and craves what is harming it, which any sick person recognises. So the bitterness is not manufactured; it is what the appetite reports once it has been partly restored, and the argument is settled by the reader's own experience rather than by assertion." },
      { title: "Take the distinction between stopping and repenting", body: "And the sharpest sentence in the section is that leaving a sin does not by itself expiate it. Leaving theft does not expiate theft. Which cuts against the natural assumption that ceasing is the whole of the return — and it explains why the definition needed three parts, since stopping is only the present-facing third of the last of them." },
    ],
    closer: [
  ...
  makeChapter({
    ...
    moves: [
      ...
      { title: "Keep it within reach", body: "The instruction is not despair but enumeration. The passage asks for the effort within one’s capacity, and then addresses absent people and cases where disclosure would cause fresh harm." },
      { title: "Take the arithmetic seriously", body: "The reason given for doing this work is presented as simple accounting, and it is worth not softening. If a man's good deeds do not cover the wrongs standing against him, the shortfall is met from the other side, and he is destroyed by other people's evils. Which makes unrepaired wrongs not a stain on his record but a liability that draws on it — and one whose size he did not set." },
      { title: "Note that the instruction is enumeration", body: "And the practical instruction is deliberately unromantic: take the wrongs one at a time, find the people wherever they have got to, and either get released or pay. Not a state to be entered, but a list to be worked. Which is what keeps the section from producing despair — the work is considered one right at a time, within capacity, while absence and harmful disclosure require separate treatment." },
    ],
    closer: [
    ...
    misreading: "The return is owed now and repair remains work to undertake. This is not a blanket instruction to reveal hidden wrongs: Ghazali considers disclosures that would cause fresh harm, as well as people who cannot be reached.",
  ...
  makeChapter({
    ...
    moves: [
      ...
      { title: "Note why the case is worth raising", body: "The question is whether a man can genuinely repent of one sin while persisting in another he knows to be wrong — and it matters because almost everybody is in that position. Ghazali holds it is conceivable: a person may truly believe what he is repenting of to be wrong while remaining overcome elsewhere. Which means the partial return is real rather than a pretence, and a reader in that state is not disqualified from having repented at all." },
      { title: "Take the harder variants", body: "And he presses into the cases that make it difficult: where the capacity for a sin has gone rather than the desire, and where two men repenting differ only in whether the pull toward the sin has quieted. Those are the cases where a person cannot tell from inside whether he has repented or merely run out of opportunity — and where the scholars differ, he gives the positions rather than a formula." },
    ],
    closer: [
  ...
  makeChapter({
    ...
    moves: [
      ...
      { title: "Take the second class as the ordinary case", body: "And the second class is where most serious people actually live, which is worth registering because it is described without alarm. He keeps to the main obediences and leaves the great sins, and is not free of sins that befall him — not deliberately and without a formed prior resolve — and each time he blames himself, grieves, and renews the resolve. That is named as a class of the repentant rather than as a failure of repentance." },
      { title: "Note where the line is drawn", body: "And the line falls between the third class and the fourth, not where a reader would expect. The third still regrets, still says he will repent and struggle — and defers, day after day, because his soul makes it look reasonable. The fourth returns and does not regret. Which means the disqualifying thing is not the recurrence or even the deferral, but the disappearance of the regret that the definition put at the centre." },
    ],
    closer: [
  ...
  makeChapter({
    ...
    moves: [
      ...
      { title: "Note that recurrence is treated as normal", body: "The section assumes the sin will come back rather than treating its return as the collapse of the enterprise — which follows directly from the second and third classes being ordinary conditions rather than failures. That assumption is what makes the instruction usable: it is written for the state a person is actually in on the day he needs it." },
      { title: "Take the two exits it closes", body: "And it refuses both available exits. Despair would end the striving, which is what the fourth class amounts to. And deferral keeps the intention alive precisely so the act need not occur, which is the third class. What is prescribed instead is narrow and immediate: renew the repentance, and drive the bad deed off with a good one — a decision about the next hour rather than a verdict on the past months." },
    ],
    closer: [
  ...
  makeChapter({
    ...
    moves: [
      ...
      { title: "Note the method stated in full", body: "The method is the Ihya's method, stated here more plainly than anywhere else: there is no cure without a treatment, no treatment until you know the illness, and a treatment is nothing but the contradiction of whatever caused it. Anything with a cause is cured by breaking the cause, and nothing is undone except by its opposite. Which is why the section's next move is to name the causes rather than to prescribe anything." },
      { title: "Take the two causes and their two opposites", body: "And the causes come to two: heedlessness and appetite. Nothing opposes heedlessness but knowledge, and nothing opposes appetite but patience in cutting off what stirs it. Which is why the two saving qualities that occupy the rest of this quarter are knowledge and patience, and why a programme built on effort alone keeps failing — effort is not the opposite of either cause." },
      { title: "Follow the image of the compound", body: "And the remedy is a single preparation kneaded from two things that do not go together: the sweetness of knowledge and the bitterness of patience, like oxymel made of sugar and vinegar, each aimed at something different and the two together subduing what neither would subdue alone. Which is the argument against taking either half on its own — knowledge without patience diagnoses and does not treat, and patience without knowledge endures the wrong thing." },
    ],
    closer: [
  ...
  makeChapter({
    ...
    moves: [
      ...
      { title: "Note what the four assents actually establish", body: "The four are the conditions under which anybody at all can be treated, borrowed from medicine and applied exactly. That illness and health have causes you can reach. That this particular physician knows his art and tells the truth. That his general warnings are worth being afraid of. And that his account of your own case in particular is worth attending to. A patient missing any one of the four will not complete a course of treatment, however good the medicine." },
      { title: "Take the first assent seriously", body: "And the first is the one people skip. Believing in medicine amounts to believing that being ill and being well both have causes a person can get at — and somebody who does not believe that will never begin. The counterpart is stated flatly: happiness has a cause, which is doing what you were asked, and misery has one too. A reader who does not hold that is not going to be reached by anything in this quarter." },
      { title: "Note where the fourth assent leads", body: "And the fourth is where the book ends up and where it hands the reader on. Not every patient must avoid everything, and not every remedy helps every case — each ailment has its own knowledge and its own treatment. Which is why repentance opens the quarter rather than closing it: the general return has been established, and the particular ailments are what the remaining eight books are for." },
    ],
    closer: [
